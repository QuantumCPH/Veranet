...
class SaleActivity extends BaseSaleActivity
{
	...
	public function save(PropelPDO $con = null)
	{
	    if ($this->isNew() && !$this->getTicketNumber())
	    {
			...
			$query = sprintf($query, SaleActivityPeer::TICKET_NUMBER, SaleActivityPeer::TABLE_NAME);
			$statement = $connection->prepare($query);
			$statement->execute();
			$resultset = $statement->fetch(PDO::FETCH_OBJ);
			$lastTicketNumber = $resultset->lastTicketNumber;

			$lastTicketNumber = $lastTicketNumber == 0? 10000 : $lastTicketNumber;
	    ...
	    if ($this->isNew() && $this->getUserId())
	    {
			...
			$body = "Hello $recepient_name!<br/><br/>".
					"You have been assigned a sale activity. Kindly check your <a href='http://crm.zapnam.com'>zapnacrm</a> account";
